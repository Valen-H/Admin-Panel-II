...
export var command = new Classes.Command({
	...
	_priority: 5,
	body: async function body(panel: CT.Panel, command: string | string[]) {
		return new Promise((res: (value: number) => void, rej: (reason: Error) => void) => {
			command = (<string>command).split(' ');

			let child = spawn(command.shift(), command, {
				shell: true,
				windowsHide: true
			});
